...
import {formatDateTime, getId} from "../utils/common.js";
...
export const createTripEventEditTemplate = (tripEvent, eventTypes) => {
  ...
  const eventTypeOffers = eventTypes.find((it) => {
    return it.name === type.name;
  });

  const createOffersMarkup = (eventOffers) => {
    return eventOffers.map((it)=>{
      const isChecked = offers.find((element) => {
        return element.description === it.description;
      });

      const id = getId(it.description);
  ...
  const offersMarkup = createOffersMarkup(eventTypeOffers.offers);
  ...
  return (`<li class="trip-events__item"><form class="event  event--edit" action="#" method="post">
    <header class="event__header">
      ...
      <input id="event-favorite-1" class="event__favorite-checkbox  visually-hidden" type="checkbox" name="event-favorite" ${ isFeatured ? `checked` : ``}>
